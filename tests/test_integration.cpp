...
TEST_CASE("Integration - AUTOPORTS declaration order grouping", "[integration][formatting]") {
    auto top_sv = getFixturePath("grouping_declaration_autoports/top.sv");
    auto lib_dir = getFixturePath("grouping_declaration_autoports/lib");
    ...
    // Ports should appear in submodule declaration order, not grouped by direction.
    // submod order: clk (in), data_out (out), rst_n (in), valid (out), data_in (in)
    auto pos_clk      = result.modified_content.find(" clk");
    auto pos_data_out = result.modified_content.find(" data_out");
    auto pos_rst_n    = result.modified_content.find(" rst_n");
    auto pos_valid    = result.modified_content.find(" valid");
...
TEST_CASE("Integration - AUTOPORTS alphabetical grouping", "[integration][formatting]") {
    auto top_sv = getFixturePath("grouping_declaration_autoports/top.sv");
    auto lib_dir = getFixturePath("grouping_declaration_autoports/lib");
    ...
    InlineConfig cfg;
    cfg.grouping = PortGrouping::Alphabetical;
    tool.setInlineConfig(top_sv, cfg);
    ...
    // All ports sorted alphabetically regardless of direction:
    // clk, data_in, data_out, rst_n, valid
    auto pos_clk      = result.modified_content.find(" clk");
    auto pos_data_in  = result.modified_content.find(" data_in");
    auto pos_data_out = result.modified_content.find(" data_out");
    auto pos_rst_n    = result.modified_content.find(" rst_n");
    ...
    CHECK(pos_clk < pos_data_in);
    CHECK(pos_data_in < pos_data_out);
    CHECK(pos_data_out < pos_rst_n);
    CHECK(pos_rst_n < pos_valid);
}

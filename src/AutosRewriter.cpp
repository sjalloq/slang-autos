...
AutosRewriter::collectModuleInfo(const ModuleDeclarationSyntax& module) {
    ...
    if (module.header->ports &&
        ...
        if (hasMarkerInTokenTrivia(ansi.closeParen, "/*AUTOPORTS*/")) {
            ...
            info.autoports_on_closeparen = true;
        }
...
void AutosRewriter::queueAutoportsExpansion(
    ...
    // If nothing to generate, we still need to preserve the /*AUTOPORTS*/ marker.
    // If the marker was on a port (not closeParen), removing ports would lose it.
    // In that case, we need to replace the entire port list with one that has just the marker.
    if (inputs.empty() && outputs.empty() && inouts.empty()) {
        if (!info.autoports_on_closeparen && !info.autogenerated_ports.empty()) {
            // Marker was on a port - replace entire port list with empty one that has the marker
            std::string wrapper = "module _wrapper_(/*AUTOPORTS*/\n);\nendmodule\n";
            auto& parsed = parse(wrapper);
            ...
            // Replace the entire port list with the new empty one (preserving the marker)
            if (parsed_mod && parsed_mod->header->ports) {
                replace(*info.ansi_ports, *parsed_mod->header->ports);
            }
        }
        // If marker was on closeParen, removing ports doesn't affect it - just return
        return;
    }
    ...
    // Replace the entire port list with the new one
    // Use preserveTrivia=true to keep original formatting around the opening paren
    replace(*info.ansi_ports, *parsed_mod->header->ports, true);
}
